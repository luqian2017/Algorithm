...
public:
    ...
    vector<int> goodsTransfer(int n, vector<int> &a, vector<int> &b) {
        ...
        dp[0] = 0;
        for (int i = 0; i < n; i++) {
            for (int j = sum_cap; j > 0; j--) { //注意不是j>=b[i]
                if (j <= b[i]) {
                    if (dp[j] > 1) {
                        dp[j] = 1; //如果b[i]的容量>j，则1个包就够 dp[j]=1
                        weight[j] = a[i]; //取a[i]为weight[j]
                    } else if (dp[j] == 1) { //如果dp[j]=1，取weight[j]和a[i]之间最大值
                        weight[j] = max(weight[j], a[i]);
                    }
                } else {
                    int res = max(j - b[i], 0);
                    //取backpack[i]后dp个数更多，则忽略
                    //if (dp[j] < dp[res] + 1) continue;
                    if (dp[j] > dp[res] + 1) {
                        dp[j] = dp[res] + 1; //取backpack[i]后dp个数更少
                        weight[j] = weight[res] + a[i]; //
                    } else if (dp[j] == dp[res] + 1) { //取backpack[i]后dp个数一样，则看哪个weight多
